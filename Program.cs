    ...
    class Program
    {
        ...
        */
        ...
        private static string RemoveVowels(string s)
        {
		 ...
		 // storing the vowels in the list
		List<char> al = vowels.OfType<char>().ToList();;
		// Initialize the StringBuilder with string s
		StringBuilder sb = new StringBuilder(s);
		
		for (int i = 0; i < sb.Length; i++) {
		// checking if the list contains the characters in the string
			if(al.Contains(sb[i])){
                               // if list contains the characters in the string then replace string with empty string
       ...
       */
        ...
        private static bool ArrayStringsAreEqual(string[] bulls_string1, string[] bulls_string2)
        {
            ...
            {
                // write your code here.
                if(bulls_string1.ToUpper() == bulls_string2.ToUpper()) //comparing both strings 
            {
                return true;
            }
                return false;
        ...
        */
		...
		{
		   int flag=0;
		   //Console.WriteLine("i value"+i+"number flowing"+bull_bucks[i]);
			for(int j=0;j<bull_bucks.Length;j++)
			{
				if(bull_bucks[i]==bull_bucks[j] && i!=j)
				{
					//Console.WriteLine("entered number"+bull_bucks[i]);
					flag=1;
				}
			}
		   if(flag==0)
		   {
			...
			Console.WriteLine("SumOfUnique= "+sum);
                return 0;
        ...
        */
        ...
        private static int DiagonalSum(int[][] bulls_grid)
        {
            ...
            {
                // write your code here.
               int primarySum = 0, secondarySum = 0, n = bulls_grid.Length; //initialize variables
        
        	for(int i=0, j=0; i<n; i++,j++) //loop for primarysum
        	{
        	    primarySum += bulls_grid[i][j];
        	}
        
        	for(int i=0, j=n-1; i<n; i++,j--)//loop for secondarysum
        	{
        ...
        */
        ...
        private static string RestoreString(string bulls_string, int[] indices)
        {
            ...
            {
                // write your code here.
                int n = bulls_string.Length; //length of input string
		string output = ""; //empty string to store output
                string[] str = new string[n];
                
                for (int i = 0; i < n; i++) //looping through the given indices and storing the respective value of bulls_string in the str
                {
                    int a = indices[i];
                    str[a] = Convert.ToString(bulls_string[i]);
				
                }
				
                for (int i = 0; i < n; i++) // looping through str and concatenating it for final output
                {
                    output = output + str[i];
                }
                return output; 
        ...
        */
        ...
        private static string ReversePrefix(string bulls_string6, char ch)
        {
            try
            {
                string[] rev = new string[bulls_string.Length];
                for (int i = 0; i < bulls_string.Length; i++)//looping through the given input and splitting the input into 2 parts with given ch input
                {
                    if (bulls_string[i] == ch)
                    {
                        rev = bulls_string.Split(ch);
			break;
                    }
                }
                ...
                // looping through each character from right to left of reversible string
                for(int i = rev[0].Length - 1; i > -1; i--) 
                 { 
         
                          // Append each character to the reversedstring.
                          reversedString += bulls_string[i]; 
                 }
     
                 // Return the reversed string.
	         String prefix_string= ch + reversedString + rev[1];
